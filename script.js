...
// New DOM elements for manage tab and modals
        ...
        .then(() => {
          ...
          update(ref(db, `users/${childKey}`), updatedUserData)
            .then(() => {
              ...
              loadUsers()
              loadCases()
              loadManageStudents() // Load data for manage tab
...
function displayManageStudents(studentsArray) {
  ...
  studentsArray.forEach((student) => {
    ...
    const points = student.points || student.poin || 0
    const status = getPointsStatus(points)
...
// Global function for delete modal
window.showDeleteModal = (studentId, studentName) => {
  studentToDeleteId = studentId
  studentToDeleteElement.textContent = studentName || "Siswa ini"
...
function deleteStudent(studentId) {
  showDialog("Menghapus data siswa...")
  ...
  set(userRef, null)
    ...
    .then((snapshot) => {
      ...
      if (cases) {
        const studentData = allStudentsForManage.find((s) => s.id === studentId)
        const studentUID = getProperUID(studentData, studentId)

        Object.keys(cases).forEach((caseId) => {
          if (cases[caseId].uid === studentUID) {
            deletePromises.push(set(ref(db, `cases/${caseId}`), null))
          }
        })
      }
...
function updateResetValueDisplay() {
...
function confirmResetPoints() {
  ...
  if (isNaN(resetValue) || resetValue < 0 || resetValue > 100) {
    showDialog("Nilai poin harus antara 0-100")
    return
  }

  resetAllPoints(resetValue)
  hideResetModal()
}

function resetAllPoints(newPoints) {
  showDialog("Mereset poin semua siswa...")

  if (allStudentsForManage.length === 0) {
    showDialog("Tidak ada siswa untuk direset")
    return
  }

  const updatePromises = []

  allStudentsForManage.forEach((student) => {
    const updateData = {
      ...student,
      points: newPoints,
      poin: newPoints,
    ...
    updatePromises.push(update(ref(db, `users/${student.id}`), updateData))
  })

  Promise.all(updatePromises)
    .then(() => {
      showDialog(`Berhasil mereset poin ${allStudentsForManage.length} siswa ke ${newPoints}!`)
      loadUsers()
      loadManageStudents()
      loadCases()
    })
